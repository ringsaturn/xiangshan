...
// minIndexSegments is the minimum ring length required to build a YStripes index.
...
// Closing duplicate points (exterior[0] == exterior[n-1]) are stripped.
// YStripes indices are built for rings with at least minIndexSegments unique points.
...
// exterior ring and outside all hole rings. Points on a ring boundary return false.
...
// Exterior returns the exterior ring in open form.
func (poly *Polygon) Exterior() Ring {
	return poly.exterior
}

// Holes returns the interior hole rings in open form.
