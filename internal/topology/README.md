...
Topology-aware polygon simplification for division boundary data.

## Overview

Standard Douglas-Peucker simplification treats each polygon independently. When
two adjacent division polygons share a boundary, simplifying them separately
...
Rings below the configured subtype area threshold are passed through unchanged. Segments shorter than `minSegmentSimplifyPoints = 4` are also passed through unchanged.
