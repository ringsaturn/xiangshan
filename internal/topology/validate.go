...
func Validate(input *Dataset) error {
	return ValidateWithOptions(input, DefaultValidateOptions())
}

func ValidateWithOptions(input *Dataset, opts ValidateOptions) error {
	if input == nil {
		return nil
	}
	...
	for divisionIdx, division := range input.Divisions {
		for polygonIdx, polygon := range division.Polygons {
			if err := validateRing(
				ringRef{DivisionIdx: divisionIdx, PolygonIdx: polygonIdx, HoleIdx: -1},
				polygon.Points,
				false,
				opts,
				edgeIndex,
			...
			for holeIdx, hole := range polygon.Holes {
				if err := validateRing(
					ringRef{DivisionIdx: divisionIdx, PolygonIdx: polygonIdx, HoleIdx: holeIdx},
					hole.Points,
					true,
					opts,
					edgeIndex,
...
func MustValidate(input *Dataset) {
	if err := Validate(input); err != nil {
		panic(err)
	}
}

func MustValidateForReduction(input *Dataset) {
...
func pointOnClosedSegment(point, from, to *Point) bool {
