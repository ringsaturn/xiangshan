...
// deduplication for division boundary data.
package topology
...
const (
	DefaultMinRingPoints     = 4
	minSegmentSimplifyPoints = 4
)
...
func (s Stats) String() string {
	...
	return fmt.Sprintf(
		"topology_rings: total=%d no_fixed=%d one_fixed=%d multi_fixed=%d fallback=%d\n"+
			"topology_rings_skipped: small=%d\n"+
...
func Do(input *Dataset, epsilon float64) *Dataset {
	output, _ := DoWithStats(input, epsilon)
	return output
}

func DoWithStats(input *Dataset, epsilon float64) (*Dataset, Stats) {
	return DoWithOptions(input, Options{Epsilon: epsilon})
}

func DoWithOptions(input *Dataset, opts Options) (*Dataset, Stats) {
	stats := Stats{}
	if input == nil {
		return nil, stats
	}
	if opts.MinRingPoints <= 0 {
		opts.MinRingPoints = DefaultMinRingPoints
	}

	output := normalizeDivisions(input)
	...
	snapVertices(output, opts.MinRingArea, opts.MinRingPoints, &stats)
	...
	rings, edgeIndex, vertexIndex := collectRings(output, opts.MinRingArea, opts.MinRingPoints)
	...
	recordFixedVertices(rings, &stats)
	sharedCache := make(map[sharedSegmentKey][]*Point)

	for ref, ring := range rings {
		simplified := simplifyRing(ring, opts.Epsilon, sharedCache, &stats)
...
func removeZeroLengthEdges(input *Dataset) {
	for _, tz := range input.Divisions {
...
func normalizeWindings(input *Dataset) {
	for _, division := range input.Divisions {
		for _, polygon := range division.Polygons {
...
func normalizeRingWinding(points []*Point, isHole bool) []*Point {
	if len(points) == 0 {
		return nil
	}
...
func cleanRing(points []*Point) []*Point {
	if len(points) == 0 {
		return nil
	}
...
func snapVertices(input *Dataset, minRingArea float64, minRingPoints int, stats *Stats) {
	...
	ringPoints := map[ringRef][]*Point{}
	ringVertices := map[ringRef]map[pointKey]struct{}{}
	allVertices := make([]vertexRef, 0)

	for divisionIdx, division := range input.Divisions {
		for polygonIdx, polygon := range division.Polygons {
			ref := ringRef{DivisionIdx: divisionIdx, PolygonIdx: polygonIdx, HoleIdx: -1}
			points := ringUniquePoints(polygon.Points)
			if !ringSimplifiable(points, minRingArea, minRingPoints) {
				continue
			}
			...
			for holeIdx, hole := range polygon.Holes {
				holeRef := ringRef{DivisionIdx: divisionIdx, PolygonIdx: polygonIdx, HoleIdx: holeIdx}
				holePoints := ringUniquePoints(hole.Points)
				if !ringSimplifiable(holePoints, minRingArea, minRingPoints) {
					continue
				}
	...
	for ref, points := range ringPoints {
		...
		rebuilt := make([]*Point, 0, len(points)+len(insertions))
...
func normalizeDivisions(input *Dataset) *Dataset {
	output := &Dataset{
	...
	for _, division := range input.Divisions {
		...
		for _, polygon := range division.Polygons {
			newPolygon := &Polygon{
				...
				Holes:  make([]*Polygon, 0, len(polygon.Holes)),
			}
			for _, hole := range polygon.Holes {
				newPolygon.Holes = append(newPolygon.Holes, &Polygon{
					Points: normalizeRing(hole.Points),
				})
			}
			newDivision.Polygons = append(newDivision.Polygons, newPolygon)
		}
		output.Divisions = append(output.Divisions, newDivision)
	}
	return output
}

func normalizeRing(points []*Point) []*Point {
	if len(points) == 0 {
		return nil
	}
	normalized := make([]*Point, 0, len(points)+1)
	for _, point := range points {
...
func normalizePoint(point *Point) *Point {
	if point == nil {
		return nil
	}
	...
	for divisionIdx, division := range input.Divisions {
		for polygonIdx, polygon := range division.Polygons {
			addRing(rings, edgeIndex, vertexIndex, ringRef{
				...
				HoleIdx:     -1,
			}, polygon.Points, minRingArea, minRingPoints)
			for holeIdx, hole := range polygon.Holes {
				addRing(rings, edgeIndex, vertexIndex, ringRef{
					DivisionIdx: divisionIdx,
					PolygonIdx:  polygonIdx,
					HoleIdx:     holeIdx,
				}, hole.Points, minRingArea, minRingPoints)
			}
		}
	}
...
func addRing(
	...
	rings[ref] = &ringData{
		...
		Simplifiable: ringSimplifiable(unique, minRingArea, minRingPoints),
...
func ringSimplifiable(unique []*Point, minRingArea float64, minRingPoints int) bool {
	return len(unique) >= minRingPoints && math.Abs(signedArea(unique)) >= minRingArea
}
...
func markSharedEdges(rings map[ringRef]*ringData, edgeIndex map[edgeKey][]edgeUse) {
	for _, uses := range edgeIndex {
		...
		rings[uses[0].Ring].Edges[uses[0].EdgeIdx] = edgeMeta{
			...
			PartnerSimplifiable: rings[uses[1].Ring].Simplifiable,
		}
		rings[uses[1].Ring].Edges[uses[1].EdgeIdx] = edgeMeta{
			Shared:              true,
			PartnerRing:         uses[0].Ring,
			PartnerSimplifiable: rings[uses[0].Ring].Simplifiable,
		}
	}
}
...
func recordFixedVertices(rings map[ringRef]*ringData, stats *Stats) {
	if stats == nil {
		return
	}
	for _, ring := range rings {
...
func cleanRingRemoveZeroEdges(points []*Point) []*Point {
	unique := ringUniquePoints(points)
	if len(unique) < 3 {
		return points
	}
	...
	cleaned := make([]*Point, 0, len(unique))
...
func ringHasZeroLengthEdge(unique []*Point) bool {
	for i, p := range unique {
		if samePoint(p, unique[(i+1)%len(unique)]) {
			return true
		}
...
func getOriginalRing(input *Dataset, ref ringRef) []*Point {
	division := input.Divisions[ref.DivisionIdx]
	poly := division.Polygons[ref.PolygonIdx]
	if ref.HoleIdx == -1 {
		return poly.Points
	}
...
// whose shape exactly matches another division's exterior polygon.
...
func findCanonicalStart(points []*Point) int {
	best := 0
...
func simplifyRing(ring *ringData, epsilon float64, sharedCache map[sharedSegmentKey][]*Point, stats *Stats) []*Point {
	points := ring.Points
	fixed := ring.Fixed
	if len(points) == 0 {
		return nil
	...
	if !ring.Simplifiable {
		if stats != nil {
			stats.RingsSkippedSmall++
		}
		return cloneRing(points)
	}
	...
	case 0:
		if isEntirelyShared(ring.Edges) {
			...
			// enclave: a hole in an outer division whose shape matches the inner
			// division's exterior). Rotate to the lexicographically smallest
			...
			openPath := make([]*Point, 0, len(rotated)+1)
	...
	case 1:
		...
		if isEntirelyShared(rotatedEdges) {
			...
			if stats != nil && len(openPath) <= minSegmentSimplifyPoints {
...
func simplifyFixedSegments(
	...
	fixed []int,
	epsilon float64,
	sharedCache map[sharedSegmentKey][]*Point,
	stats *Stats,
) []*Point {
	if len(points) == 0 {
		return nil
	}
	assembled := make([]*Point, 0, len(points)+1)
	for idx := range fixed {
		start := fixed[idx]
		...
		segment := make([]*Point, 0, end-start+1)
...
func simplifySegment(
	...
	if !segmentSimplifiable(segmentEdges) {
		if stats != nil {
			recordSegmentStats(stats, len(segment), len(segment), false)
			stats.SegmentsSkippedShort++
		}
	...
	if !ok {
		...
		if stats != nil && len(segment) <= minSegmentSimplifyPoints {
			stats.SegmentsSkippedShort++
		}
		return reduced
	}
...
func segmentSimplifiable(segmentEdges []edgeMeta) bool {
	for _, edge := range segmentEdges {
		if edge.Shared && !edge.PartnerSimplifiable {
			return false
		}
	}
	return true
...
func simplifyClosedRing(points []*Point, epsilon float64, stats *Stats) []*Point {
	path := make([]*Point, 0, len(points))
	...
	if stats != nil && len(path) <= minSegmentSimplifyPoints {
		stats.SegmentsSkippedShort++
	}
...
func assignRing(output *Dataset, ref ringRef, points []*Point) {
	if ref.HoleIdx == -1 {
		output.Divisions[ref.DivisionIdx].Polygons[ref.PolygonIdx].Points = points
		return
	}
	output.Divisions[ref.DivisionIdx].Polygons[ref.PolygonIdx].Holes[ref.HoleIdx].Points = points
}

func ringUniquePoints(points []*Point) []*Point {
	if len(points) == 0 {
		return nil
	}
	if len(points) == 1 {
		return []*Point{clonePoint(points[0])}
	}
...
func closeRing(points []*Point) []*Point {
	if len(points) == 0 {
		return nil
	}
...
func rotatePoints(points []*Point, start int) []*Point {
...
func reverseOpenPath(points []*Point) []*Point {
	reversed := make([]*Point, 0, len(points))
...
func segmentSignature(points []*Point) string {
	hasher := fnv.New64a()
...
func buildVertexSet(points []*Point) map[pointKey]struct{} {
...
func newPointKey(point *Point) pointKey {
	return pointKey{
...
func pointOnSegment(point, from, to *Point) (bool, float64) {
...
func pointBounds(point *Point) [2][2]float64 {
	x := float64(point.Lng)
	y := float64(point.Lat)
	return [2][2]float64{{x - snapTolerance, y - snapTolerance}, {x + snapTolerance, y + snapTolerance}}
}

func segmentBounds(from, to *Point) [2][2]float64 {
...
func cloneRing(points []*Point) []*Point {
	return clonePoints(points)
}

func clonePoints(points []*Point) []*Point {
	cloned := make([]*Point, 0, len(points))
	for _, point := range points {
